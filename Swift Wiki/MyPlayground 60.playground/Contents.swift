...
let anotherGenerator = LotteryGenerator()
anotherGenerator.timesTwo()

extension EmojiDescribable {
...
extension CollectionType where Generator.Element: Describable {
    
    var textDescription: String {
        
        let itemDescription = self.map {$0.description}
        return itemDescription.joinWithSeparator(", ")
        
    }
    
}

let harry = Dog(description: "Harry")
let rex = Dog(description: "Rex")

let dogs = [harry,rex]

dogs.textDescription
